function [fdobj, coef, df, gcv, SSE, dev, stats] = ...
...
%     Property        Value
%     ----------------------------------------------------------------
%     weight          A vector of the same length as the data vector to be
...
if n >= nbasis || lambda > 0
    ...
    %  ------------------------------------------------------------------
    %               compute solution using Matlab function glm_fda
    %  ------------------------------------------------------------------

    if ndim < 3
        [coef,dev,stats] = glm_fda(basismat, y, family, [], wtvec);
    else
        ...
        for ivar=1:nvar
            yi = y(:,:,ivar);
            [coefi,devi,statsi] = glm_fda(basismat, yi, family, [], wtvec);
            coef(:,:,ivar) = coefi;
            dev(:,ivar)    = devi;
            stats{:,ivar}  = statsi;
         end
